...
use graphene_std::text::{Font, OverflowBehavior, TextAlign};
			...
			match concrete_type.alias.as_ref().map(|x| x.as_ref()) {
				...
				_ => {
					...
					match concrete_type.id {
						...
						Some(x) if x == TypeId::of::<OverflowBehavior>() => enum_choice::<OverflowBehavior>().for_socket(default_info).property_row(),
